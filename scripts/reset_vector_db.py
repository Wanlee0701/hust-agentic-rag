...
def reset_chroma_db(confirm: bool = False):
    ...
    from scripts.build_knowledge_base import DataPreparationPipelineRefactored
    pipeline = DataPreparationPipelineRefactored(config_path="./config.yaml")
    chunks = pipeline.step_2_chunk_documents(config_path="./config.yaml")
    # Re-run step 2 to get fresh chunks, then step 3
    from src.embeddings.processor import TextChunker
    chunks = TextChunker.chunk_all_documents(pipeline.output_path, config_path="./config.yaml")
